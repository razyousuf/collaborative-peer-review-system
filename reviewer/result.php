...
if (!isset($_GET['paper_id']) || empty($_GET['paper_id'])) {
    echo 'Error: No paper ID provided.';
    exit;
}
$paper_id = intval($_GET['paper_id']);

// Example usage: Fetch grading criteria related to this paper
$criteria_query = $conn->prepare("SELECT * FROM grades WHERE paper_id = ? AND reviewer_id = ?");
$criteria_query->bind_param("ii", $paper_id, $reviewer_id);
$criteria_query->execute();
$result = $criteria_query->get_result();
if (!$paper_id) {
    die("Paper not found.");
}

echo "You are reviewing Paper ID: " . $paper_id;
...
$(document).ready(function() {
    var reviewer_id = <?= json_encode($_SESSION['login_id']); ?>;
    var paper_id = <?= json_encode($paper_id); ?>; // Dynamic paper_id from PHP
    ...
    $('.save-grades').click(function() {
        ...
        if (grades.length > 0) {
            $.ajax({
                ...
                method: 'POST',
                data: { reviewer_id: reviewer_id, grades: grades, paper_id: paper_id },
